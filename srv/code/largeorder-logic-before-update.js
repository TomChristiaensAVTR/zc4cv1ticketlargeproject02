/**
 * Before largeorder UPDATE
 * @Before(event = { "UPDATE" }, entity = "AdminService.LargeOrder")
 * @param {Object} request - User information, tenant-specific CDS model, headers and query parameters
*/
module.exports = async function(request) {
	const LargeOrder = request.entity;
	const LOG = cds.log('C4C', {level: 'warn'});
	let messagetxt = `TCH - Before UPDATE invoked`;
	...
	messagetxt = `On BEFORE Patch/Update Executed with ${RequestedID}`;
	console.log(messagetxt);
	LOG.info(messagetxt);
	...
	if (RequestedID == undefined) {
		...
		if ( RequestedID.length < 15 && RequestedID.length > 1) {
			...
			console.log(messagetxt);
			LOG.info(messagetxt);
			messagetxt = `Most probably ${RequestedID} is the C4C Ticket ID`;
			console.log(messagetxt);
			LOG.info(messagetxt);
			...
			if (record3.ID.length > 1){
				//Adapt the UPDATE request to the correct value
				...
				request.query.UPDATE.entity.ref[0].where[2].val = record3.ID;
			}        
		}
		else {
